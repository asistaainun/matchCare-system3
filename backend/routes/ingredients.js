...
router.get("/", async (req, res) => {
  try {
    const { limit = 24, offset = 0, search, category } = req.query;
    ...
    // Add search filter
    if (search) {
      query += ` AND (name ILIKE $${paramIndex} OR what_it_does ILIKE $${paramIndex} OR benefit ILIKE $${paramIndex})`;
      countQuery += ` AND (name ILIKE $${paramIndex} OR what_it_does ILIKE $${paramIndex} OR benefit ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }
    ...
    if (category && category !== "all") {
      query += ` AND (functional_categories ILIKE $${paramIndex} OR what_it_does ILIKE $${paramIndex})`;
      countQuery += ` AND (functional_categories ILIKE $${paramIndex} OR what_it_does ILIKE $${paramIndex})`;
...
router.get("/:ingredientName", async (req, res) => {
  try {
    ...
    const ingredientDetails =
      await ontologyService.getIngredientDetails(ingredientName);
    ...
    try {
      ...
      const testCombinations = commonCombinations
        .filter((ing) => ing.toLowerCase() !== ingredientName.toLowerCase())
        .slice(0, 3);

      if (testCombinations.length > 0) {
        const [conflicts, synergies] = await Promise.all([
          ontologyService.getIngredientConflicts([
            ingredientName,
            ...testCombinations,
          ]),
          ontologyService.getSynergisticCombos([
            ingredientName,
            ...testCombinations,
          ]),
        ]);
    ...
    const productsWithIngredient =
      await router.findProductsWithIngredient(ingredientName);
  ...
  try {
    ...
    if (!ingredients || !Array.isArray(ingredients) || ingredients.length < 2) {
      return res.status(400).json({
        success: false,
        message:
          "Please provide at least 2 ingredients for compatibility check",
        ontology_powered: true,
      });
    }

    console.log(
      `🔬 Ontology compatibility check for: ${ingredients.join(", ")}`
    );
    ...
    const compatibilityReport = {
      ...
      conflict_analysis: {
        ...
        risk_level: router.calculateRiskLevel(
          conflicts.count,
          ingredients.length
        ),
      ...
      synergy_analysis: {
        ...
        enhancement_potential: router.calculateEnhancementPotential(
          synergies.count
        ),
        beneficial_combinations: router.extractBeneficialCombinations(
          synergies.data
        ),
      },

      overall_assessment: {
        compatibility_score: router.calculateCompatibilityScore(
          conflicts.count,
          synergies.count,
          ingredients.length
        ),
        recommendation: router.generateOverallRecommendation(
          conflicts.count,
          synergies.count
        ),
        usage_guidance: router.generateUsageGuidance(
          conflicts.data,
          synergies.data
        ),
...
router.get("/benefits", async (req, res) => {
  try {
    ...
    const otherBenefits = benefits.filter(
      (b) => !categorizedBenefits.includes(b)
    );
...
router.get("/functions", async (req, res) => {
  try {
    ...
    const functionStats = functions.map((func) => ({
      function: func,
      ingredient_count: allIngredients.data.filter(
        (ing) => ing.function === func
      ).length,
...
router.calculateCompatibilityScore = function (
  conflictCount,
  synergyCount,
  ingredientCount
) {
  ...
  const synergyBonus = Math.min(
    (synergyCount / maxPossibleInteractions) * 20,
    20
  );
  ...
  const finalScore = Math.max(
    0,
    Math.min(100, baseScore - conflictPenalty + synergyBonus)
  );
...
router.generateUsageGuidance = function (conflicts, synergies) {
  ...
  if (conflicts.length > 0) {
    guidance.push(
      "⚠️ Separate conflicting ingredients by time (AM/PM) or days"
    );
...
router.calculateEnhancementScore = function (synergies) {
  ...
  const qualityBonus =
    synergies.filter((s) => s.recommendation && s.recommendation.length > 20)
      .length * 5;
...
router.getBestCombinations = function (synergies) {
  return synergies
    .sort(
      (a, b) =>
        (b.recommendation?.length || 0) - (a.recommendation?.length || 0)
    )
    .slice(0, 5)
    .map((synergy) => ({
...
router.generateSynergyUsageTips = function (synergies) {
  ...
  if (
    synergies.some(
      (s) =>
        s.name1.toLowerCase().includes("acid") ||
        s.name2.toLowerCase().includes("acid")
    )
  ) {
...
router.suggestAlternatives = function (conflicts) {
  ...
  conflicts.forEach((conflict) => {
    if (
      conflict.name1.toLowerCase().includes("retinol") &&
      conflict.name2.toLowerCase().includes("acid")
    ) {
      alternatives.push("Use retinol at night and acids in the morning");
    } else if (
      conflict.name1.toLowerCase().includes("vitamin c") &&
      conflict.name2.toLowerCase().includes("retinol")
    ) {
      alternatives.push("Use Vitamin C in AM and retinol in PM");
    } else {
      alternatives.push(
        `Alternate ${conflict.name1} and ${conflict.name2} on different days`
      );
    }
  });
  ...
  return alternatives.length > 0
    ? alternatives
    : ["Consider using ingredients separately or consulting an expert"];
};
...
router.testDatabaseConnection = async function () {
  try {
    ...
    const result = await client.query(
      "SELECT COUNT(*) FROM ingredients WHERE is_active = true"
    );
    client.release();
    console.log(
      `✅ Database connected: ${result.rows[0].count} active ingredients found`
    );
    return true;
  } catch (error) {
