// backend/scripts/selective-update-ingredients.js
// Script untuk UPDATE kolom-kolom tertentu saja dari CSV baru
...
class SelectiveIngredientUpdater {
    constructor() {
        this.stats = {
            csv_rows: 0,
    ...
    mapCSVToDatabase() {
        return {
            ...
            'keyIngredientTypes': 'key_ingredient_types',
            ...
            fs.createReadStream(csvFilePath)
                ...
                .on('data', (row) => {
                    ...
                    if (this.stats.csv_rows <= 3) {
                        console.log(`\n🔍 CSV Row ${this.stats.csv_rows} sample:`, {
                            name: row.name,
                            explanation: (row.explanation || '').substring(0, 50) + '...',
                            benefit: (row.benefit || '').substring(0, 30) + '...',
                            isKeyIngredient: row.isKeyIngredient
                        });
                    }
                    ...
                    if (this.stats.csv_rows % 5000 === 0) {
                        console.log(`📊 Processed ${this.stats.csv_rows} CSV rows...`);
                    }
                })
                .on('end', async () => {
                    try {
                        console.log(`\n📋 CSV processing complete: ${this.stats.csv_rows} rows, ${updates.length} valid ingredients`);
    ...
    async performSelectiveUpdate() {
        console.log('🔄 MatchCare Selective Ingredient Update');
        console.log('=======================================\n');
...
// Run the update
async function main() {
    const updater = new SelectiveIngredientUpdater();
...
module.exports = SelectiveIngredientUpdater;